...
//			}
			...
			function insertPoint(latlon, startPoint) {
				...
				const wp = decodePolyline(route.paths[0].snapped_waypoints.coordinates);

				if (wpts.length > 4) {
					alert('Only 5 waypoints allowed:\nGraphhopper limitation');
				...
				while (j >= 0) {
					...
					for (i = 0; i < coords.length-1; i++) {
						const d = L.latLng(wp[j]).distanceTo(coords[i]);
						if (d < minDist) {
							minIndex2 = i;
							minDist = d;
						}
					}
					way_points.push(minIndex2);
					j--;
				}
				j = way_points.length - 1;
				way_points = way_points.reverse();
				while (j >= 0 && way_points[j] >= minIndex) {
					j=j-1;
				}
...
//					geoRequest(e.latlng, 0, 'ins');
			...
			function loadRoute(request) {
				...
				for (let i = 0; i < route.paths[0].instructions.length; i=i+1) {
						let stepDistance = route.paths[0].instructions[i].distance;
						if (stepDistance > 1609) stepDistance = `${(stepDistance / 1609.34).toFixed(2)} miles`;
						else stepDistance += 'm';
						p = L.DomUtil.create('p', 'route-info', routeInfo);
						p.innerHTML = `- ${route.paths[0].instructions[i].text} (${stepDistance})`;
						addRowListener(route.paths[0].instructions[i], coords);
...
//					if (ferry[0].checked) ferry = '"ch.disable": true,'
//					+ '"custom_model": {"priority": [{"if": "road_environment == FERRY","multiply_by": "0.0"}]}'
//					else ferry = '"ch.disable": true,'
//					+ '"custom_model": {"priority": [{"if": "road_environment == FERRY","multiply_by": "1.0"}]}';
...
//					}).toString();
					...
					request.open('POST', `https://graphhopper.com/api/1/route?key=${ghAPI}`);
...
//			}
			...
			const profiles = { Walk: 'foot', Car: 'car', Bike: 'bike' };
			let checked;

			const values = Object.values(profiles);
			const keys = Object.keys(profiles);
			let key;
			if (profile === 'foot-walking') profile = 'foot'
			else if (profile === 'driving-car') profile = 'car'
			else if (profile === 'cycling-road') profile = 'bike';
...
//			pref.innerHTML = '<label><input type="checkbox" id="other_prefs" name="ferry" checked>Avoid ferries</label>';
