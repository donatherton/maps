...
function geodesicConvertLine(startLatlng, endLatlng, convertedPoints) {    
  ...
  let d, segments, f, A, B, x, y, z, fLat, fLng;
  ...
  for (let i = 1; i <= segments; i++) {
    ...
    fLat = toDegrees * Math.atan2(z, Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2)));
...
export default function (latlngs, options) {
  return new GeodesicPolyline(latlngs, options);
};
